...
class SqlForm extends Component {
    ...
    handleValidSubmit = (event,values) => {
        ...
        axios.post("http://localhost:5000/createDatabase",data)
            .then(response => {
                console.log("Got the response from create database",response.data);
                    ...
                    axios.post("http://localhost:5000/database",data)
                        .then(response1 => {
                            ...
                            if(response1.status === 200){
                                ...
                                bodyFormData.append('file', this.state.sqlFile);
                                bodyFormData.append('hostname',values.hostname)
                                bodyFormData.append('username',values.username)
                                bodyFormData.append('password',values.password) 
                                axios.post(" http://localhost:5000/sql-upload/"+data.database,bodyFormData)
                                    .then(response =>{
                                        console.log("SQL API RESPONSE IS: : ",response.data)
                                        if(response.status === 200){
    ...
    handleAcceptedFiles = files => {
        var sqlFile = this.state.sqlFile
        files.map(file => {
            sqlFile = file
            // console.log(file)
        });
